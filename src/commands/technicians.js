...
const TECH_COLUMNS = ['id', 'first_name', 'last_name', 'email', 'phone'];
const TECH_HEADERS = {
  id: 'ID',
  ...
  techs
    ...
    .action(async (options) => {
      ...
      try {
        ...
        output(items, { format: globalOpts.output, columns: TECH_COLUMNS, headers: TECH_HEADERS });
      } catch (err) {
  ...
  techs
    ...
    .action(async (id) => {
      ...
      try {
        ...
        outputDetail(tech, { format: globalOpts.output, headers: TECH_HEADERS });
      } catch (err) {
        console.error(chalk.red(err.message));
        process.exitCode = 1;
      }
